      ...
      <header className={`${styles.header} sticky top-0 z-50`} role="banner">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between gap-4">
          ...
          <Link 
            ...
            aria-label="TechJobsIL - חזרה לדף הבית"
          >
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center gap-1 shrink-0" role="navigation" aria-label="ניווט ראשי">
            ...
            {/* Auth buttons */}
            <div className={`ml-2 pl-2 border-l ${styles.divider}`} role="group" aria-label="אזור משתמש">
              {isLoading ? (
                <div 
                  className={`w-8 h-8 rounded-full ${isDark ? 'bg-white/20' : 'bg-warm-100'} animate-pulse`}
                  role="status"
                  aria-label="טוען מידע משתמש"
                />
                ...
                <Button
                  ...
                  aria-label="התחבר עם חשבון Google"
                >
          ...
          <>
            ...
            >
              <div className="flex flex-col h-full">
                ...
                <nav className="flex-1 p-4 space-y-2" role="navigation" aria-label="ניווט ראשי">
                  {navLinks.map(({ path, icon: Icon, label, checkActive }) => (
                    <Link
                      key={path}
                ...
                {/* Mobile Auth Section */}
                <div className="p-4 border-t border-warm-200" role="group" aria-label="אזור משתמש">
                  {isLoading ? (
                    <div 
