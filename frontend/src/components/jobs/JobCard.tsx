          ...
          <CardContent className="p-6">
            <div className="flex justify-between items-start gap-4">
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap" role="list" aria-label="Job attributes">
                      <Badge
                        variant="outline"
                      ...
                      <Badge variant="outline" className="bg-iris-50 text-iris-700 border-iris-200 text-xs" role="listitem">
                        {job.job_category}
                      </Badge>
                      {isSaved && (
                        <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-100 border-emerald-200 gap-1 pl-1.5" role="listitem">
            ...
            <div className="flex flex-col gap-2" role="group" aria-label="Job actions">
              <Button
                ...
                size="icon"
                onClick={() => onSave(job)}
                aria-label={isSaved ? `הסר את ${job.title} מהמשרות השמורות` : `שמור את ${job.title}`}
              ...
              {onApply && (
                <Button
                  ...
                  size="icon"
                  onClick={() => onApply(job)}
                  aria-label={isApplied ? `בטל סימון הגשה עבור ${job.title}` : `סמן כהוגש עבור ${job.title}`}
            ...
            <div className="flex items-center gap-2">
              ...
              >
                <a 
                  ...
                  aria-label={`הגש מועמדות ל-${job.title} ב-${job.company} (נפתח בחלון חדש)`}
                >
                  Apply Now
