  ...
  return (
    ...
    >
      <Link to={`${createPageUrl("CompanyProfile")}?name=${encodeURIComponent(company.name)}`}>
        <Card className="h-full hover:shadow-lg transition-all duration-300 hover:border-indigo-200 group cursor-pointer">
          <CardContent className="p-6 flex flex-col h-full">
            <div className="flex justify-between items-start mb-4">
              <CompanyLogo name={company.name} className="w-12 h-12 bg-slate-50 group-hover:bg-white transition-colors" />
